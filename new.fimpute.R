...
gradient.trevor <- function(B,W,y,index){
...
gradient.old.lukasz = function(B, W, y, index){
  n = nrow(W)
  # Get model predictions on observed timepoints
  model_preds = lapply(1:n, function(i) {
    idx = (index == i)
    B[idx,] %*% W[i,]
  })
  model_preds = unlist(model_preds)
  
  # Calculate residuals
  residuals = y - model_preds
  res = sum(residuals**2)
  
  # Inner product
  grad = t(sapply(1:n, function(i) {
    idx = (index == i)
    residuals[idx] %*% B[idx,]
  }))
  list(grad=grad, res=res)
}

fimpute.fit = function(data,
                         value.vars,
                         time.var,
                         id.var,
                         basis = create.bspline.basis(nbasis=7),
                         niter = 50,
                         pp = 10,
                         df = 7,
                         lr = 0.05,
  ...
  grid = seq(rangeval[1], rangeval[2],length.out = 100)
  # basis = bs(grid,df = df,degree = degree)
#  basis = create.bspline.basis(rangeval = rangeval, nbasis=7)

  basis = fda::create.polygonal.basis(rangeval = rangeval)
  df = 2
...
#                                       data_long[[time.var]][slices[[value.var]]])
  ...
  for (i in 1:niter){
    ...
    for (j in 1:nvars){
      ...
      res.update = gradient.trevor(basis_evals[[value.var]],
                                   Wold[,((j-1)*df+1):(j*df)],
